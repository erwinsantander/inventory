...
if (isset($_POST['update'])) {
    $req_fields = array('name', 'username', 'alternative_email');
    validate_fields($req_fields);
    if (empty($errors)) {
        $id = (int)$_SESSION['user_id'];
        $name = remove_junk($db->escape($_POST['name']));
        $username = remove_junk($db->escape($_POST['username']));
        $alternative_email = remove_junk($db->escape($_POST['alternative_email']));
        
        $sql = "UPDATE users SET name ='{$name}', username ='{$username}', alternative_email ='{$alternative_email}' WHERE id='{$id}'";
        $result = $db->query($sql);
            ...
            <div class="panel-body">
                <form method="post" action="edit_account.php?id=<?php echo (int)$user['id']; ?>" class="clearfix">
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" class="form-control" name="alternative_email" value="<?php echo remove_junk($user['alternative_email']); ?>">
                    </div>
